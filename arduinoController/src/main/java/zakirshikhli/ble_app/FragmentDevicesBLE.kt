...
class FragmentDevicesBLE : ListFragment() {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        @SuppressLint("InflateParams") val header =
            requireActivity().layoutInflater.inflate(R.layout.device_list_header_ble, null, false)
    ...
    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) {
        ...
        } else if (!bluetoothAdapter!!.isEnabled || (!gpsEnabled && !networkEnabled)) {
            menu.findItem(R.id.ble_scan).setVisible(false)
            menu.findItem(R.id.bt_refresh_ble).setVisible(true)
        }
    }
    ...
    override fun onResume() {
        ...
        if (bluetoothAdapter == null) {
            ...
            setEmptyText(getString(R.string.location_is_disabled))
            if (menu != null) {
                listItems.clear()
                listAdapter!!.notifyDataSetChanged()
                menu!!.findItem(R.id.ble_scan).setVisible(false)
    ...
    private fun startScan() {
        ...
        if (scanState == ScanState.LE_SCAN) {
            leScanStopHandler.postDelayed(leScanStopCallback, BLE_SCAN_PERIOD)
    ...
    private fun updateScan(device: BluetoothDevice) {
        ...
        val deviceScanResult = BLEutil.Device(device) // slow getName() only once

        deviceName = deviceScanResult.name
        ...
        val pos = Collections.binarySearch(listItems, deviceScanResult)
        if (pos < 0) {
            listItems.add(-pos - 1, deviceScanResult)
            listAdapter!!.notifyDataSetChanged()
        }
    }
    ...
    private fun stopScan() {
        if (scanState == ScanState.NONE) return
        setEmptyText(getString(R.string.nothingFound))
        if (menu != null) {
            menu!!.findItem(R.id.ble_scan).setVisible(true)
    ...
    override fun onListItemClick(l: ListView, v: View, position: Int, id: Long) {
        ...
        val fragment: Fragment = FragmentController()
        fragment.arguments = args
        assert(fragmentManager != null)
        requireFragmentManager().beginTransaction().replace(R.id.fragment, fragment, "controller")
            .addToBackStack(null).commit()
    }


    companion object {
        private const val BLE_SCAN_PERIOD: Long =
